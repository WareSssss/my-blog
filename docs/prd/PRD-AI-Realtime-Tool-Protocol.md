...
为了确保团队对核心概念达成共识，以下是对文档中关键术语的解释：

- **RTP (Real-time Tool Protocol)**: 
  本项目定义的“实时工具调用协议”。它是一套标准化的契约，规定了 AI 模型（Client）如何发现、调用以及接收来自工具服务器（Tool Server）的实时反馈。其核心目标是解决 AI 能力集成的碎片化问题。

- **MCP (Model Context Protocol)**: 
  由 Anthropic 推出的开放标准。它允许开发者通过标准化的方式为大型语言模型（LLM）提供上下文数据（如文件、数据库、Slack 消息等）。RTP 在设计思路上借鉴了 MCP 的解耦理念，但更侧重于工具执行的实时性与双向通信。

- **DDD (Domain-Driven Design)**: 
  领域驱动设计。一种处理复杂软件设计的思维方式，强调通过建立统一语言（Ubiquitous Language）将业务专家与开发团队连接起来。本文档使用 DDD 来划分“核心域”、“支撑域”与“限界上下文”，以确保系统架构与业务逻辑高度对齐。

- **JSON-RPC 2.0**: 
  一种无状态、轻量级的远程过程调用（RPC）协议。它定义了请求、响应及通知的结构，使用 JSON 作为数据交换格式。RTP 选择它作为传输层协议，因其简单且原生支持异步通知。

- **SSE (Server-Sent Events)**: 
  服务器发送事件。一种基于 HTTP 的单向通信技术，允许服务器向客户端推送实时更新。RTP 利用 SSE 实现工具执行进度的流式反馈，比 WebSocket 更轻量且易于维护。
