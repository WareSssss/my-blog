...
### 2.2 项目配置 (针对 Monorepo 最佳实践)
在 "Set up builds and deployments" 页面进行如下配置：

- **Framework preset**: 选择 `None` (不要选 Vite，我们要手动配置命令)。
- **Build command**: `pnpm build:web` 
  *(说明：这会调用根目录的脚本，确保能正确处理 Monorepo 依赖)*
- **Build output directory**: `apps/web/dist`
  *(说明：Vite 构建后的产物所在位置)*
- **Root directory**: `/` 
  *(重要：必须保持在根目录，否则无法找到 pnpm-lock.yaml)*

### 2.3 设置环境变量 (Environment Variables)
在部署页面下方的 "Environment variables" 栏目添加：

| 变量名 (Variable name) | 值 (Value) | 说明 |
| :--- | :--- | :--- |
| `NODE_VERSION` | `18` | 确保 Node 版本足够高 |
| `PNPM_VERSION` | `9` | 确保 pnpm 版本与本地一致 |
